...
export function loadStaticAppAPIInlineSvgIcon() {
...
export function isSvgContentType(ct) {
...
function loadExAppInlineSvgIcon(appId, route) {
	...
	return axios.get(url, {
		...
		if (!isSvgContentType(ct)) {
			console.debug('[app_api] icon not svg', { url, status: response.status, ct })
			return null
		}
		// If status is 304 or body is empty, do not block the action.
		if (!response.data) {
			console.debug('[app_api] icon empty body', { url, status: response.status, ct })
			return null
		}
		return response.data
	}).catch((error) => {
		console.error('[app_api] Failed to load ExApp FileAction icon inline svg', {
			url,
			status: error?.response?.status,
			ct: error?.response?.headers?.['content-type'],
			error,
		})
		return null
	})
...
export function themeInlineSvg(svg) {
	try {
		const parser = new DOMParser()
		const doc = parser.parseFromString(svg, 'image/svg+xml')

		if (doc.getElementsByTagName('parsererror').length > 0) {
			console.warn('[app_api] SVG parse error, using fallback icon')
			return null
		}
		...
		root.setAttribute('style', `${existing}${needsSemicolon ? ';' : ''} filter: var(--background-invert-if-dark);`)

		return root.outerHTML
	} catch (e) {
		console.error('[app_api] Failed to theme SVG, using fallback icon', e)
		return null
	}
}
...
export function generateExAppUIPageUrl(appId, route) {
...
export function buildNodeInfo(node) {
	return {
		fileId: node.fileid,
		name: node.basename,
...
function init() {
	const fallbackIcon = loadStaticAppAPIInlineSvgIcon()

	state.fileActions.forEach((fileAction) => {
		// Always register immediately with fallback icon
		let icon = fallbackIcon
		const iconProvider = () => icon

		registerFileAction33(fileAction, iconProvider)
