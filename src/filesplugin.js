...
if (OCA.Files && OCA.Files.fileActions) {
	...
	state.fileActions.forEach(fileAction => {
		const action = new FileAction({
			...
			iconSvgInline: () => fileAction.icon_class === 'icon-app-api' ? loadStaticAppAPIInlineSvgIcon() : null, // TODO: Rewrite fileActions to use proxy to get SvgInline from ExApps
			order: Number(fileAction.order),
			enabled(nodes) {
				if (nodes.length !== 1) {
					return false
				...
				return (nodes[0].mime.indexOf(fileAction.mime) !== -1)
			},
			async exec(node) {
				axios.post(generateOcsUrl('/apps/app_api/api/v1/files/action'), {
					appId: fileAction.appid,
