OC.L10N.register(
    ...
    "reported a problem" : "сообщает о проблеме",
    ...
    "AppAPI default deploy daemon is not set. Please register a default deploy daemon in the settings to install External Apps (Ex-Apps)." : "Служба развертывания AppAPI по умолчанию не установлена. Пожалуйста, зарегистрируйте службу развертывания по умолчанию в настройках для установки внешних приложений (Ex-Apps).",
    ...
    "The AppAPI default deploy daemon is not using HaRP. Please consider switching to HaRP for better performance." : "Демон развертывания AppAPI по умолчанию не использует HaRP. Рекомендуется перейти на HaRP для повышения производительности.",
    "External Apps (Errors)" : "Внешние приложения (Ошибки)",
    "External Apps (Warnings)" : "Внешние приложения (Предупреждения)",
    "AppAPI HaRP version check" : "Проверка версии AppAPI HaRP",
    "Could not retrieve HaRP version from daemon \"%s\"" : "Не удалось получить версию HaRP из демона \"%s\"",
    "HaRP version for daemon \"%s\" is \"%s\", which is too old. The minimum required version is \"%s\". Please update the daemon to the latest version." : "Версия HaRP для демона \"%s\" — \"%s\", она слишком старая. Минимально требуемая версия — \"%s\". Обновите демон до последней версии.",
    "Failed to check HaRP version for daemon \"%s\": %s" : "Не удалось получить версию HaRP для демона \"%s\": %s",
    "Nextcloud AppAPI" : "Nextcloud AppAPI",
    "### What is AppAPI?\n\nAppAPI is the Nextcloud component that enables **External Apps (ExApps)**.\nExApps are apps whose backend runs **outside** the Nextcloud PHP process (usually as a Docker container), while still integrating with your Nextcloud users, permissions and web UI.\n\n### Do I need it?\n\nNo. If you do not plan to install External Apps, you can disable AppAPI.\n\nIf AppAPI is enabled without a configured Deploy Daemon, Nextcloud will show the info message:\n\n> \"AppAPI default deploy daemon is not set …\"\n\nThat warning means: \"External Apps are not configured yet\".\n\nYou can either disable AppAPI, or configure a Deploy Daemon (see below).\n\n### How do External Apps get deployed?\n\nA **Deploy Daemon** is the service Nextcloud talks to in order to install, start, stop and reach ExApps.\n\nTwo options are available:\n\n- **HaRP** (recommended for Nextcloud 32+) — a high-performance reverse proxy with FRP tunneling\n- **Docker Socket Proxy** (legacy) — scheduled for removal in Nextcloud 35\n\nSee the [admin documentation](https://docs.nextcloud.com/server/latest/admin_manual/exapps_management/DeployConfigurations.html) for setup instructions." : "### Что такое AppAPI?\n\nAppAPI — это компонент Nextcloud, обеспечивающий работу **Внешних приложений (ExApps)**.\nExApps — это приложения, бэкенд которых работает **вне** PHP-процесса Nextcloud (обычно в виде контейнера Docker), но при этом интегрируется с пользователями, правами доступа и веб-интерфейсом Nextcloud.\n\n### Нужно ли мне это?\n\nНет. Если вы не планируете устанавливать внешние приложения, вы можете отключить AppAPI.\n\nЕсли AppAPI включен, но не настроен демон развертывания, Nextcloud отобразит информационное сообщение:\n\n> «Демон развертывания AppAPI по умолчанию не настроен…»\n\nЭто предупреждение означает: «Внешние приложения ещё не настроены».\n\nВы можете либо отключить AppAPI, либо настроить демон развертывания (см. ниже).\n\n### Как происходит развертывание внешних приложений?\n\n**Демон развертывания** — это служба, с которой Nextcloud взаимодействует для установки, запуска, остановки и доступа к внешним приложениям.\n\nДоступны два варианта:\n\n- **HaRP** (рекомендуется для Nextcloud 32+) — высокопроизводительный обратный прокси с туннелированием FRP\n- **Docker Socket Proxy** (устаревший) — планируется удалить в Nextcloud 35\n\nИнструкции по настройке см. в [документации для администраторов](https://docs.nextcloud.com/server/latest/admin_manual/exapps_management/DeployConfigurations.html).",
    ...
    "Deploy daemons" : "Службы развёртывания",
    "A deploy daemon (DaemonConfig) is an ExApps orchestration daemon." : "Демон развертывания (DaemonConfig) — это демон оркестрации ExApps.",
    "Deploy daemons. A deploy daemon (DaemonConfig) is an ExApps orchestration daemon." : "Развертывание демонов. Демон развертывания (DaemonConfig) — это демон оркестрации ExApps.",
    "Default deploy daemon is not accessible. Please check its configuration" : "Доступ к демону развертывания по умолчанию отсутствует. Пожалуйста, проверьте его настройки",
    ...
    "Choose the container restart policy, e.g. 'always' to ensure ExApps will be running after a daemon server reboot" : "Выберите политику перезапуска контейнера, например 'всегда', чтобы гарантировать, что ExApps будут работать после перезапуска сервера-демона",
    "This settings changes are effective only for newly created containers" : "Эти изменения настроек действуют только для вновь создаваемых контейнеров",
    "Deploy daemon deletion confirmation" : "Подтверждение удаления демона развёртывания",
    "Are you sure you want delete the \"{name}\" deploy daemon?" : "Вы уверены, что хотите удалить демон развертывания \"{name}\"?",
    "Remove all ExApps installed on this daemon" : "Удалить все приложения ExApps, установленные на этом демоне",
    "Cancel" : "Отмена",
    "Delete" : "Удалить",
    "A \"manual-install\" deploy daemon cannot be set as default" : "Служба развертывания \"manual-install\" не может быть установлен по умолчанию",
    "Failed to save admin settings. Check the logs" : "Не удалось сохранить параметры администратора. Проверьте логи",
    ...
    "Failed to connect to the daemon. Check the logs" : "Не удалось установить соединение с демоном. Проверьте логи",
    "Failed to check connection to the daemon. Check the logs" : "Не удалось проверить соединение с демоном. Проверьте логи",
    "Unlimited" : "Неограничено",
    "{size} GiB" : "{size} ГиБ",
    "{size} MiB" : "{size} МиБ",
    "_{n} CPU_::_{n} CPUs_" : ["{n} ЦП","{n} ЦП","{n} ЦП","{n} ЦП"],
    "Deploy daemon configuration" : "Конфигурация службы развёртывания",
    "Deploy daemon configuration details" : "Подробная конфигурация демона развёртывания",
    "Deploy daemon" : "Служба развёртывания",
    ...
    "GPU support" : "Поддержка GPU",
    "Computation device" : "Вычислительное устройство",
    ...
    "All ExApps on this daemon will be removed" : "Все ExApps на этой службе будут удалены.",
    "All ExApps installed on this daemon will be removed" : "Все ExApps, установленные на этой службе, будут удалены.",
    ...
    "All available GPU devices on daemon host will be requested to be enabled in ExApp containers by Docker." : "Docker запросит включение всех доступных устройств GPU на хосте службы в контейнерах ExApp.",
