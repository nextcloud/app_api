...
def scenario_keep_image_cancels_stale_pending_job() -> None:
    print("scenario 6: --keep-image cancels a job queued by an earlier uninstall")
    deploy_app()
    assert_image_eventually_present(IMAGE_NAME_GREP)
    ...
    # Reinstall before the queued job fires, then uninstall again asking to KEEP
    # the image. The stale job from the first uninstall must be cancelled, or it
    # would delete the image the admin explicitly asked to keep.
    deploy_app()
    occ("app_api:app:unregister", APP_ID, "--keep-image")
    pending = list_pending_jobs("OrphanedImageCleanupJob")
    assert pending == [], f"--keep-image should cancel the stale queued job, got {pending}"
    time.sleep(2)
    assert image_present(IMAGE_NAME_GREP), "--keep-image should leave the image in place"


def scenario_purge_now_overrides_disabled_master_toggle() -> None:
    print("scenario 7: --purge-now is honored even when automatic cleanup is disabled")
    occ("config:app:set", "app_api", "image_cleanup_enabled", "--value=false", "--type=boolean")
    try:
        deploy_app()
        assert_image_eventually_present(IMAGE_NAME_GREP)
...
def main() -> int:
    ...
    try:
        ...
        reset_state()
        scenario_keep_image_cancels_stale_pending_job()

        reset_state()
        scenario_purge_now_overrides_disabled_master_toggle()
    finally:
        reset_state()
