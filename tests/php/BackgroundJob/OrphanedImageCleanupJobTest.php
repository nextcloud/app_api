...
class OrphanedImageCleanupJobTest extends TestCase {
	...
	private ExAppImageCleanupService&MockObject $imageCleanupService;
	...
	protected function setUp(): void {
		...
		$this->imageCleanupService = $this->createMock(ExAppImageCleanupService::class);
		$this->imageCleanupService->method('isMasterEnabled')->willReturn(true);
	...
	public function testNonArrayArgumentLogsWarningAndReturns(): void {
		$this->logger->expects(self::once())
			->method('warning')
			->with(self::stringContains('non-array argument'));
		$this->daemonConfigService->expects(self::never())->method('getDaemonConfigByName');

		$this->invokeRun('not-an-array');
	}
	...
	public function testMasterDisabledSkipsWithoutCallingRemove(): void {
		$imageCleanupService = $this->createMock(ExAppImageCleanupService::class);
		$imageCleanupService->method('isMasterEnabled')->willReturn(false);
		$job = new OrphanedImageCleanupJob(
			$this->time,
			$this->daemonConfigService,
			$this->dockerActions,
			$imageCleanupService,
		...
		$this->logger->expects(self::once())
			->method('info')
			->with(self::stringContains('cleanup is disabled'));

		$reflection = new \ReflectionMethod($job, 'run');
		$reflection->setAccessible(true);
		$reflection->invoke($job, ['daemon_id' => 'docker-harp', 'image_ref' => 'r:1', 'appid' => 'demo']);
	}
